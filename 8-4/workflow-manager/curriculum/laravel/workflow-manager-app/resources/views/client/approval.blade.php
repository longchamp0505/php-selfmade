        ...
        <table>
            ...
            <tbody>
                @foreach($attendanceData as $data)
                    ...
                    $isSubmittedButNotOther = $att && ($att->is_submitted ?? 0) && !($att->is_other_company_work ?? 0);
                    @endphp
                    ...
                    <tr>
                        ...
                        {{-- 区分 --}}
                        <td>
                            {{ $att ? ($isSubmittedButNotOther ? '非稼働' : ($categoryMap[$att->category ?? ''] ?? '')) : '' }}
                        </td>

                        {{-- 始業打刻 --}}
                        <td>{{ $att && !$isSubmittedButNotOther && $att->clock_in ? \Carbon\Carbon::parse($att->clock_in)->format('H:i') : '' }}</td>
                        <td>{{ $att && !$isSubmittedButNotOther && $att->start_time ? \Carbon\Carbon::parse($att->start_time)->format('H:i') : '' }}</td>

                        {{-- 終業打刻 --}}
                        <td>{{ $att && !$isSubmittedButNotOther && $att->clock_out ? \Carbon\Carbon::parse($att->clock_out)->format('H:i') : '' }}</td>
                        <td>{{ $att && !$isSubmittedButNotOther && $att->end_time ? \Carbon\Carbon::parse($att->end_time)->format('H:i') : '' }}</td>

                        {{-- 休憩 --}}
                        <td>{{ $att && !$isSubmittedButNotOther && $att->break_time ? \Carbon\Carbon::parse($att->break_time)->format('H:i') : '' }}</td>

                        {{-- 実働／契約外 --}}
                        <td>{{ $att && !$isSubmittedButNotOther && $data['workMinutes'] > 0 ? $minutesToTime($data['workMinutes']) : '' }}</td>
                        <td>{{ $att && !$isSubmittedButNotOther && $data['overtimeMinutes'] > 0 ? $minutesToTime($data['overtimeMinutes']) : '' }}</td>

                        {{-- 備考 --}}
                        <td>{{ $att && !$isSubmittedButNotOther ? ($att->remarks ?? '') : '' }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    approveBoxes.forEach(box => {
        box.addEventListener('change', function() {
            ...
            if(this.checked){
                const rejectBox = row.querySelector('input[name="rejected[]"]');
                rejectBox.checked = false;
            }
            updateRowColor(row);   // ここで色を即時更新
            sendRowUpdate(row);
        });
    });
    ...
    rejectBoxes.forEach(box => {
        box.addEventListener('change', function() {
            const row = this.closest('tr');
            if(this.checked){
                const approveBox = row.querySelector('input[name="approved[]"]');
    ...
    okBtn.addEventListener('click', function() {
        ...
        if(box){
            ...
            box.checked = true;      // モーダル OK で確定
            updateRowColor(row);     // 色を即時更新
            sendRowUpdate(row);
        }
